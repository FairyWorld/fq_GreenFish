...
class sub_merge():
    def sub_merge(url_list): # 将转换后的所有 Url 链接内容合并转换 YAML or Base64, ，并输出文件，输入订阅列表。
        ...
        content_yaml = sub_convert.convert(content_all,'content','YAML',{'dup_rm_enabled': True, 'format_name_enabled': True, 'speedtest': True})
        ...
        write_list = [f'./url', f'./Subscription/GreenFishPool', f'./Subscription/Clash/GreenFishPool']
    ...
    def readme_update(readme_file='./README.md', sub_list=[]): # 更新 README 节点信息
        ...
        for index in range(len(lines)):
            if lines[index] == '### 高速节点\n': # 目标行内容
                ...
                with open('./Subscription/GreenFishPool', 'r', encoding='utf-8') as f:
