...
class sub_convert():
    ...
    def convert(raw_input, input_type='url', output_type='url', custom_set={'dup_rm_enabled': False, 'format_name_enabled': False, 'speedtest': False}):
        if input_type == 'url':
            sub_content = ''
    ...
    def makeup(input, dup_rm_enabled=True, format_name_enabled=True, speedtest=True):
        ...
        for proxy in proxies_list:
            if format_name_enabled:
                ...
                if len(proxies_list) > 999:
                    proxy['name'] = f'{name_emoji}{country_code}(绿头外网集团)(Pool)'
    ...
    def convert_remote(url='', output_type='clash', host='https://api.v1.mk'):
        sever_host = host
